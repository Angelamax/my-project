...
export default async function handler(req, res) {
  ...
  if (!url) {
    return res.status(400).json({ success: false, author, message: "Masukkan parameter 'url'!" });
  }
  ...
  try {
    ...
    // 1. Ambil Token
    const response1 = await axios.get(TARGET_URL, { headers: { 'User-Agent': userAgent } });
    ...
    if (data.status !== "ok" || (data.mess && data.mess.includes("privado"))) {
        return res.status(200).json({
            success: false,
            author,
            message: "Gagal: Video ini bersifat privat atau tidak didukung oleh server.",
    ...
    const results = [];
    if (data.data) {
       const $api = cheerio.load(data.data);
       $api('a.download-link-fb').each((i, el) => {
           const quality = $api(el).closest('tr').find('.video-quality').text().trim();
           const dUrl = $api(el).attr('href');
           if (dUrl) results.push({ quality, url: dUrl });
       });
    }
